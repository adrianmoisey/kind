...
func planAddNodes(cfg *config.Cluster, networkName string, existingNames []string, binaryName string) (createContainerFuncs []func() error, newNames []string, err error) {
	...
	genericArgs, err := commonArgs(cfg.Name, cfg, networkName, allNames, binaryName)
	if err != nil {
		return nil, nil, err
	}
	...
	for i, node := range cfg.Nodes {
		...
		// fixup relative paths, nerdctl can only handle absolute paths
		for m := range node.ExtraMounts {
			hostPath := node.ExtraMounts[m].HostPath
			if !fs.IsAbs(hostPath) {
				absHostPath, err := filepath.Abs(hostPath)
