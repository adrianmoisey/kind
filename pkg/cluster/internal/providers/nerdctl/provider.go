...
func (p *provider) CreateNodes(status *cli.Status, cfg *config.Cluster) (created []nodes.Node, err error) {
	...
	// ensure the pre-requisite network exists
	if err := ensureNetwork(fixedNetworkName, p.Binary()); err != nil {
		return nil, errors.Wrap(err, "failed to ensure nerdctl network")
	}
	...
	createContainerFuncs, newNames, err := planAddNodes(cfg, fixedNetworkName, existingNames, p.Binary())
	if err != nil {
		return nil, err
	}
...
func (p *provider) defaultNodeImages(cfg *config.Cluster, existing []nodes.Node) error {
	...
	image, err := nodeImage(kubeNodes[0].String(), p.Binary())
	if err != nil {
...
func nodeImage(name, binaryName string) (string, error) {
	cmd := exec.Command(binaryName, "inspect", "--format", "{{.Config.Image}}", name)
